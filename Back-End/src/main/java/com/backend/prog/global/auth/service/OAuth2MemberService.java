...
public class OAuth2MemberService {
    ...
    private final List<String> ADJECTIVE_LIST = Arrays.asList("행복한", "우울한", "쓸쓸한", "무거운", "따뜻한", "재밌는", "훌륭한", "잘생긴", "어여쁜", "귀여운", "친절한", "순수한", "상냥한",
            "정직한", "성실한", "공정한", "뚱뚱한", "우아한", "즐거운", "이상한", "유명한", "수줍은", "특별한", "피곤한", "솔직한", "심각한", "활발한", "무례한",
            "격분한", "무서운", "어색한", "가엾은", "실망한", "대담한", "용감한", "화려한", "건방진", "강력한");

    private final List<String> TERM_LIST = Arrays.asList("고양이", "강아지", "거북이", "토끼", "뱀", "사자", "호랑이", "표범", "치타", "하이에나", "기린", "코끼리", "코뿔소", "하마", "악어",
    ...
    public MemberSecurityDTO loadMember(String provider, String email, String username) {
        ...
        else if(memberOptional.get().getProvider() == null || (memberOptional.get().getProvider() != null && !provider.equals(memberOptional.get().getProvider().getProvider()))){
            new UsernameNotFoundException("already");
        }
        else {
    ...
    public String randomNickname() {
        ...
        log.info(ADJECTIVE_LIST.size());
        log.info(TERM_LIST.size());

        while (true) {
            nickname = ADJECTIVE_LIST.get(random.nextInt(0, ADJECTIVE_LIST.size())) + TERM_LIST.get(random.nextInt(0, TERM_LIST.size()));
            int len = random.nextInt(1, 7);

            for(int i = 0; i < len; i++) {
                nickname += random.nextInt(0, 10);
            }
